...
exports.findById = function (id) {
  return articleModle.findOne({
    _id: id
  }).populate('catagory', 'name').lean().exec();
}
...
exports.edit = function (id, obj) {
  ...
  return articleModle.where({
    '_id': id
  }).update({$set: obj}).populate('catagory', 'name').exec()
}
...
exports.findByLimit = function (page, limit) {
  return articleModle.find().populate('catagory', 'name').sort({'create_time': -1}).skip(page*limit).limit(limit).lean().exec();
}